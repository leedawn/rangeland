  ...
  >
    <div class="pagination-box">
      <a
        ...
        @click.prevent="home()"
      >
        <i class="angle double left icon" v-if="showType === 'icon'"></i>
        <template v-else>首页</template>
      </a>
      <a class="ui label" :class="{'disabled': current === 0}" @click.prevent="prev()">
        <i class="angle left icon" v-if="showType === 'icon'"></i>
      ...
      <a
        class="ui label"
        :class="{'disabled': current === pages.length - 1}"
        @click.prevent="next()"
      >
        <i class="angle right icon" v-if="showType === 'icon'"></i>
      ...
      <a
        ...
        @click.prevent="end()"
      >
        <i class="angle double right icon" v-if="showType === 'icon'"></i>
...
export default {
  ...
  mounted () {
    ...
    console.log('mounted -> limit', this.limit)
    this.initPages()
  ...
  methods: {
    initPages () {
      ...
      console.log('initPages -> pages', this.pages)
    },
    ...
    next () {
      ...
      if (this.current + 1 > this.pages.length - 1) {
        cur = this.pages.length - 1 /// /应该是当前页+1超出总页数的时候，取最后一页的值
      } else {
        cur = this.current + 1
      }
      this.$emit('changeCurrent', cur)
