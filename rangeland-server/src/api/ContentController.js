...
async function deletePostByUid (ctx) {
    const { body } = ctx.request
    const obj = await getJWTPayload(ctx.header.authorization)
    const post = await Post.findById(body.tid)
    if (post.uid === obj._id && post.isEnd === '0') {
        await Post.deleteOne({ _id: body.tid })
        ctx.body = {
            code: 200,
            msg: '删除帖子成功'
        }
...
module.exports = { getPostList, uploadImg, addPost, updatePost, getPostDetail, getPostListByUid, deletePostByUid }